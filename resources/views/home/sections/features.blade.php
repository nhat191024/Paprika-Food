...
<section class="w-full bg-[#eeeeee] px-8 py-20 text-center dark:bg-zinc-900">
    <div class="mx-auto max-w-7xl">
        ...
        <div class="grid grid-cols-1 gap-8 md:grid-cols-3" data-animate="stagger-children">
            @foreach($featuredProducts as $index => $product)
                <a href="{{ route('product.details', ['product_slug' => $product->slug]) }}" wire:navigate class="flex flex-col items-center bg-white p-6 dark:bg-zinc-800 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700/50 cursor-pointer hover:shadow-md transition-shadow">
